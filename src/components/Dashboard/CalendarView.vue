<template>
  ...
  <div class="space-y-6">
    ...
    <div
      ...
      class="flex gap-6 items-start lg:items-stretch"
    >
      <!-- Calendar -->
      <div
      ...
      <!-- Calendar Grid -->
      <div class="grid grid-cols-7 relative z-0" :class="selectedDate ? 'pointer-events-none lg:pointer-events-auto' : ''">
        <div
        ...
        >
          ...
          <div v-if="isDateTodayOrFuture(date.fullDate) && getPostsForDate(date.fullDate).length > 0" class="flex flex-wrap gap-1 mt-1">
            <img
              v-for="(post, idx) in getPostsForDate(date.fullDate).slice(0, 2)"
              :key="post.id"
              :src="post.postImage"
              :alt="`Post ${idx + 1}`"
              class="w-4 h-4 md:w-8 md:h-8 rounded object-cover"
            />
            <span
              v-if="getPostsForDate(date.fullDate).length > 2"
              class="label_3_medium sub_text_color"
            >
          ...
          <div v-else-if="!isDateTodayOrFuture(date.fullDate) && getPostsForDate(date.fullDate).length > 0" class="absolute bottom-0 border_none">
            <p class=" md:disable_text_color md:paragraph_p7_medium paragraph_sm disable_text_color">
              {{ getPostsForDate(date.fullDate).length }} {{ getPostsForDate(date.fullDate).length === 1 ? 'Post' : 'Posts' }}
            </p>
          </div>
        </div>
      </div>
    ...
    <div
      v-if="selectedDate && activeView === 'Month'"
  ...
  <button @click="nextWeek"> <img :src="CircleRightArrow" alt=""></button>
      ...
      <div class="md:hidden">
        ...
        <div class="flex items-center justify-between border-b bg_primary_color secondary_button_thin gap-3">
          <div
            ...
            :key="day.day"
            @click="selectedWeekDay = day.dateString"
          ...
          >
            <p
              class="sub_button_semibold"
              :class="selectedWeekDay === day.dateString || (selectedWeekDay === null && day.isToday) 
        ...
        <div ref="mobileWeekScrollContainer" class="overflow-auto h-[64vh] hide-scrollbar">
          <div
            v-for="hour in 24"
            :key="hour"
            class="border-b relative min-h-[60px]"
          >
            <div class="flex">
              <!-- Time Label -->
              ...
              <div class="flex-1 relative">
                ...
                <div
                  v-for="post in getPostsForWeekDayAndHourMobile(hour)"
                  :key="post.id"
      ...
      <div class="hidden md:block">
        <!-- Week Header -->
        <div 
          class="grid border-b bg_primary_color"
        ...
        <div 
          ref="desktopWeekScrollContainer" 
          class="grid overflow-auto h-[47vh] hide-scrollbar"
          :class="windowWidth >= 768 ? '' : 'grid-cols-8'"
          :style="windowWidth >= 768 ? { gridTemplateColumns: weekGridTemplate } : {}"
        >
          <!-- Time Column -->
          <div class="border-r">
            <div
              v-for="hour in 24"
              :key="hour"
              class="h-24 border-b flex items-start pt-2 px-2 label_2_regular"
            >
              {{ formatHour(hour === 24 ? 0 : hour - 1) }}
            </div>
          </div>
          ...
          >
            ...
            >
              ...
              <div
                v-for="post in getPostsForWeekDayAndHour(day.dateString, hour)"
                :key="post.id"
                :data-post-id="post.id"
...
<div class="flex items-center justify-between border-b common_inner_gap">
  ...
  <div ref="dayViewScrollContainer" class="grid grid-cols-[30%_70%] md:grid-cols-[20%_80%] overflow-auto h-[60vh] md:h-[44vh] hide-scrollbar">
    <!-- Time Column -->
    <div >
      <div
        v-for="hour in 24"
    ...
    <div class="relative">
      ...
      >
        <!-- Posts for this day and hour -->
        <div
          v-for="post in getPostsForDayAndHour(hour)"
          :key="post.id"
          :data-post-id="post.id"
          class="absolute left-1 right-1 rounded-md bg_white shadow-sm primary_border_color secondary_button_thin z-10"
          :style="{
